...
- (void)addReminderForToDoItem:(NSString *)item { // 添加提醒事项
    ...
    reminder.startDateComponents = [self dateComponentsForDefaultDueDate1];
    EKAlarm *alarm = [EKAlarm alarmWithAbsoluteDate:[NSDate dateWithTimeIntervalSinceNow:36 * 3600]];
...
- (void)addEventForToDoItem:(NSString *)item { // 添加提醒事项
    if (!self.isAccessToEventStoreGranted) {
        return;
    }
...
//                [self addEventForToDoItem:object];
            } else {
...
- (EKCalendar *)calendar {
    if (!_calendar) {
        ...
        if ([filtered count]) {
            ...
            NSError *error = nil;
            BOOL calendarSuccess = [self.eventStore saveCalendar:_calendar commit:YES error:&error];
            if (!calendarSuccess) {
                // Handle error
            }
        }
...
- (NSDateComponents *)dateComponentsForDefaultDueDate1 {
    NSDateComponents *oneDayComponents = [[NSDateComponents alloc] init];
    oneDayComponents.day = 1;
